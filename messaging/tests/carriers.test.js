...
describe("ambimail GATEWAY_MAP routing", () => {
  test("a mapped recipient is routed to its own gateway domain", async () => {
    ...
    const env = {
      AMBIG_API: "ak",
      GATEWAY_MAP: "5550100101:txt.att.net",
  ...
  test("map keys normalize E.164 and 10-digit forms the same way", async () => {
    ...
    const env = {
      AMBIG_API: "ak",
      GATEWAY_MAP: "+15550100100:txt.att.net",
      CARRIER_GATEWAY: "vtext.com",
    };
    try {
      const t = createTransport(env);
      await t.send({ to: "5550100100", body: "hi" });
      await t.send({ to: "+15550100100", body: "hi" });
    } finally {
      restore();
    }
    assert.equal(recipient(calls[0]), "5550100100@txt.att.net");
    assert.equal(recipient(calls[1]), "5550100100@txt.att.net");
  });
  ...
  test("an entry with no domains degrades to the fallback", async () => {
    ...
    const env = {
      AMBIG_API: "ak",
      GATEWAY_MAP: "5550100101:,5551234567:txt.att.net",
      CARRIER_GATEWAY: "vtext.com",
    };
    try {
      const t = createTransport(env);
