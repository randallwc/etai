...
describe("toE164", () => {
  ...
  test("prefixes + for other digit runs", () => {
    assert.equal(toE164("15551234567"), "+15551234567");
    assert.equal(toE164("442079460123"), "+442079460123");
  });
...
describe("fromAmbiguousMail", () => {
  ...
  test("accepts senders on every host in the push-path list", () => {
    for (const host of [
      ...
      const msg = fromAmbiguousMail({
        data: { id: "m", from: { email: `5550100100@${host}` }, body_text: "hi" },
      });
      assert.equal(msg.from, "+15550100100", host);
    }
  });
  ...
  test("accepts from as string, address field, or sender field", () => {
    ...
    for (const variant of [
      { from: "5550100100@vtext.com" },
      { from: { address: "5550100100@vtext.com" } },
      { sender: { email: "5550100100@vtext.com" } },
      { sender: "5550100100@vtext.com" },
    ]) {
      const msg = fromAmbiguousMail({ data: { ...data, ...variant } });
      assert.equal(msg.from, "+15550100100", JSON.stringify(variant));
    }
  });
  ...
  test("falls back through body_text, text, snippet, body, subject and trims", () => {
    const d = { id: "m", from: { email: "5550100100@vtext.com" } };
  ...
  test("receivedAt uses date when created_at is missing, else now", () => {
    const d = { id: "m", from: { email: "5550100100@vtext.com" }, body_text: "hi" };
    assert.equal(
  ...
  test("externalId falls back to the event id then a uuid", () => {
    const d = { from: { email: "5550100100@vtext.com" }, body_text: "hi" };
    assert.equal(
  ...
  test("returns null for non-gateway senders and missing bodies", () => {
    ...
    assert.equal(
      fromAmbiguousMail({
        data: { id: "m", from: { email: "5550100100@vtext.com" } },
      }),
      null,
    );
    assert.equal(
...
describe("phoneFromEmail", () => {
  test("maps gateway local parts to E.164", () => {
    assert.equal(phoneFromEmail("5550100100@vtext.com"), "+15550100100");
    assert.equal(phoneFromEmail("15550100100@vtext.com"), "+15550100100");
  ...
  test("returns null off the host list or without a digit local part", () => {
    assert.equal(phoneFromEmail("5550100100@gmail.com"), null);
