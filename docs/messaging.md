...
text or voice turn arrives through a webhook, /simulate/inbound, or
the mail poller, is deduped, runs through the scheduling loop
in-process, and the reply leaves by the configured transport. Run it:
`npm start` (PORT default 4020).

MODULES
-------

  index.js      HTTP surface, env loading, per-thread queues, the
                pre-job reminder timer, and wiring.
                createService(env, overrides) keeps every piece
                injectable for tests. WORKING_BEAT_MS (1500) texts
                "On it" when a turn runs long.
  ...
  ai.js         assistant/chat intent classification plus the
                per-channel prompts; falls back to keywords on failure
                or no key.
  ...
  state.js      threads, jobs, customers, dedup set, action log;
                ...
                null.
  transports.js the wire: outbound providers (ambimail when a key is
                set, else sim stdout), inbound payload normalization,
                and the Ambiguous inbox poller.
...
---------
  ...
  GET  /messages             last 200 inbound (debug)
  GET  /healthz              { ok, transport, stub, ambiguous }

INTENTS
-------

book -> offer up to 3 numbered slots; reply picks by number, ordinal,
or time; book event, "Locked in", text contractor. reschedule -> same
flow, updateEvent. cancel -> cancelEvent, tell the other party, offer
rebook. running_late -> shift next job by delayMinutes, text client
new ETA. day_summary -> route line. other -> contractor text becomes
a task; client gets help text. Working hours 9-17, 90-minute spacing,
60-minute default. Client changes notify the contractor; contractor
changes notify the affected client.

ENV
---

  PORT 4020 . STATE_FILE . CONTRACT_PHONE (the boss, reminder target) .
  CONTRACTOR_PHONE alias . CONTRACTOR_TZ (default America/Los_Angeles)
  AMBIG_API or AMBIGUOUS_API_KEY (unset -> stub calendar, sim
  transport) . AMBIGUOUS_BASE_URL
  ...
  REMINDER_LEAD_MINUTES 30 . REMINDER_POLL_MS 60000 .
  CALENDAR_SYNC_MS 30000 . CALENDAR_SYNC_DAYS 45
  AI_CLASSIFY_TIMEOUT_MS 15000 . MAIL_POLL_SECONDS 15 .
  MAIL_POLL_LIMIT 20 . FETCH_TIMEOUT_MS 8000 .
  CALENDAR=memory forces the stub

Repo-root .env loads at startup (never overrides set vars). No public
URL needed unless registering the calendar webhook.

REMINDERS
---------

Two overlapping paths, both contractor-only: the in-process timer in
index.js watches the job store for agent-booked jobs inside
REMINDER_LEAD_MINUTES; pollRemoteReminders() polls Ambiguous
...
logged once, marked read, skipped (cap 5000 remembered ids). Dedup
key "ambmail-<uuid>". Caveats: only replies to ambimail texts arrive;
latency is one poll interval plus the carrier hop; channel is "sms"
not "imessage"; polls serialize rather than stack.
...
watch the sim transport log replies.
